...
def _setup_person(persons_dir: Path, name: str) -> Path:
    """Create a minimal person directory."""
...
def _write_heartbeat_log(person_dir: Path, entries: list[dict]) -> None:
...
def _write_cron_log(person_dir: Path, entries: list[dict]) -> None:
    """Write cron log entries."""
    cron_dir = person_dir / "state" / "cron_logs"
...
class TestActivityMessageLog:
    """Test that inter-person message logs appear in activity events."""
    ...
    async def test_message_events_returned(self, tmp_path: Path) -> None:
        ...
        app = _create_app(tmp_path, person_names=["alice", "bob"])
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            resp = await client.get("/api/activity/recent")
        data = resp.json()
        ...
        evt = msg_events[0]
        assert "alice" in evt["persons"]
        assert "bob" in evt["persons"]
        assert "Hello Bob!" in evt["summary"]
        assert evt["metadata"]["from_person"] == "alice"
        assert evt["metadata"]["to_person"] == "bob"

    async def test_message_events_with_person_filter(self, tmp_path: Path) -> None:
        shared_dir = tmp_path / "shared"
        shared_dir.mkdir(parents=True, exist_ok=True)
        now = datetime.now(timezone.utc)
        _write_message_log(shared_dir, [
        ...
        app = _create_app(tmp_path, person_names=["alice", "bob", "charlie", "dave"])
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
