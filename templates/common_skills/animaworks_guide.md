...
AnimaWorksフレームワークの仕組みと使い方を理解するための共通ナレッジ。「このシステムの使い方は？」「AnimaWorksとは？」等の質問があった場合に参照する。

## 発動条件

- ユーザーから AnimaWorks の仕組みや使い方について聞かれた場合
- 自分の動作原理について質問された場合
- 新しい社員に仕組みを教える必要がある場合
...
各 Digital Person は自分の視野・記憶・判断基準を持ち、テキストベースの会話で連携する。
...
Digital Person が起動するタイミングは3つある。
